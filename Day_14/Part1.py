...
class ModifiedMask:
    ...
    def update(self, newMask):
        self.mask = [str(x) for x in newMask]

    def doMask(self, number):
        bitwiseNumber = toBitList(number, len(self.mask))
        for index in range(len(self.mask)):
            if isDigit(self.mask[index]):
                bitwiseNumber[index] = int(self.mask[index])
        return fromBitList(bitwiseNumber)
...
with open("Data\input.txt", "r") as inputFile:
    for line in inputFile:
        ...
        memLocation = int(valueResult.group("address"))
        value = int(valueResult.group("value"))
        memoryLocations[memLocation] = modifiedMask.doMask(value)
