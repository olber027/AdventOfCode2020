...
class Waypoint:
    ...
    def rotate(self, angle):
        ...
        x = self.coords[0]
        y = self.coords[1]
        self.coords = (x*math.cos(angle) - y*math.sin(angle), x*math.sin(angle) + y*math.cos(angle))

class Ship:
    def __init__(self):
        self.coords = (0,0)

    def moveForward(self, waypoint, numberOfSteps):
        for _ in range(numberOfSteps):
            self.coords = addTuple(self.coords, waypoint.coords)
...
with open("Data\input.txt", "r") as inputFile:
    for line in inputFile:
        ...
        if command["Direction"] == "F":
            ship.moveForward(waypoint, int(command["Amount"]))
        elif command["Direction"] == "R":
            waypoint.rotate(-1*int(command["Amount"]))
        elif command["Direction"] == "L":
            waypoint.rotate(int(command["Amount"]))
        else:
            waypoint.move(convertCardinalDirectionToAngle(command["Direction"]), int(command["Amount"]))
