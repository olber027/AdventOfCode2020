...
class Bag:
    ...
    def canContain(self, contentID):
        if contentID in self.possibleContents:
            return True
        for contentTuple in self.possibleContents.values():
            if contentTuple[1].canContain(contentID):
                return True
        return False
...
goldBagTally = 0
for bagID in bags:
    if bags[bagID].canContain("shiny gold"):
        goldBagTally += 1
print(goldBagTally)
